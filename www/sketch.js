...
function draw() {
  ...
  if(click){
    ...
    for(j = 0; j < piecutter.length; j++){
      ...
      let distance = dist(piecutter[j].x, piecutter[j].y, ship.x1, ship.y1);
      // Collision
      if(distance <= piecutter[j].width){
        // Go to end
        end = true;
        score = 0;
      }
...
function mousePressed(){
  ...
  if(reload > 0){
    // If on mobile
      ...
      if(mouseX >= width - width/11.5 - width/30 && mouseX <= width - width/11.5 + width/30 && mouseY >= height - height/6 - width/30 && mouseY <= height - height/6 + width/30 && click == false){
    ...
    if(mouseX >= width - width/5 - width/30 && mouseX <= width - width/5 + width/30 && mouseY >= height - height/6 - width/30 && mouseY <= height - height/6 + width/30 && click3 && click == false){
      red2 = false;
      hyper = true;
      ship.x1 = -width;
      ship.y1 = -height;
  ...
  // Start button
  if(mouseX >= width/2 - width/25 && mouseX <= width/2 + width/25 && mouseY >= height/2 - height/25 && mouseY <= height/2 + height/25 && click){
      click = false;
  }
}
